import { ParseNode, ProductionSequence } from "../types";
...
>(
  ...
  for (const production of productionSequence) {
    const { symbol: productionSymbol, production: productionSymbols } =
      production;
    ...
    if (productionSymbol !== currentNode.symbol) {
      throw new Error(
        `Attempted to add children to a node that does not match the production key: ${productionSymbol} !== ${currentNode.symbol}`
      );
    }
    ...
    const nonTerminalNodes = childNodes.filter((node) =>
      grammar.nonTerminals.has(node.symbol as NT)
    );
